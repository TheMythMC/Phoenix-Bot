...
export default class LinkManager {
  ...
  async getDataByUUID(uuid) {
    ...
    const data = await MinecraftLinkData.findOne({ MinecraftUUID: uuid });
    if (data) {
  ...
  async getDataByDiscord(id) {
    ...
    const data = await MinecraftLinkData.findOne({ DiscordID: id });
    if (data) {
      this.cache.push(data);
      return data;
    }
  ...
  async removeDiscordFromCache(id) {
    this.cache.splice(
      this.cache.findIndex((v) => v.DiscordID === id),
      1
    );
  }
  ...
  async removeUUIDFromCache(uuid) {
    this.cache.splice(
      this.cache.findIndex((v) => v.MinecraftUUID === uuid),
      1
    );
  }
