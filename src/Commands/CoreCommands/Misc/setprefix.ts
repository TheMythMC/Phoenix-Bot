...
class SetPrefix extends Command {
  ...
  async run(message: Message, _args: string[], _client: BotCore) {
    let user;
    try {
      user = await UserData.findOne({ UserID: message.member.id }).exec();
      if (!user)
        throw new Error(
    ...
    // spawn an async process that handles updating prefix in all the servers
    _client.guilds.cache.forEach(async (guild) => {
      ...
      if (m) {
        try {
          ...
          if (m.manageable) await m.setNickname(nick);
        } catch (err) {
        ...
        } // catches EVERYTHING cuz we dont care if anything happens outside the current server
      }
    });
