...
module.exports = class extends Command {
    ...
    async run(message: Message, args: string[], client: BotCore) {
        ...
        let minigame = splitted.join(' ') ? splitted.join(' ') : 'all';

        let playerData: any = HypixelAPI.getPlayerData(playername);
        ...
        if (minigame != 'all') {
            await Object.keys(aliases.games).forEach(async game => {
                if (game === minigame) {
                    ...
                    return sendCustomMessage(message.channel, "BLUE", this.msg, `Stats for ${playerData.username} in ${game}`, "");
                }
